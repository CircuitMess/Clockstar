...
void TriFilterBanks::filter(const vector<double>& input,
    ...
    Matrix<double> a(num_filter_, filter_size_, *filter_);
    VectorFloat v(filter_size_, *input.data());

    LUDecomposition lu(a);
    lu.solve_vector(v, v);

    memcpy(output.data(), v.data(), v.getSize() * sizeof(float));
...
void MFCC::computeCC(const vector<double>& lfbe, vector<double>& cc) {
	...
	Matrix<double> a(options_.num_cepstral_coeff, options_.num_tri_filter, *dct_matrix_);
	VectorFloat v(options_.num_tri_filter, *lfbe.data());

	LUDecomposition lu(a);
	lu.solve_vector(v, v);

	memcpy(cc.data(), v.data(), v.getSize() * sizeof(float));
